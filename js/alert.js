...
(() => {
  const cfg = {
    message: "Welcome — explore my research, projects, and contact info.",
    showOnce: true,
    hideForDays: 7,
    type: "toast",
    autoClose: 0
  ...
  function daysBetween(ts) { return (Date.now() - ts) / (1000 * 60 * 60 * 24); }
  try {
    if (cfg.showOnce) {
      const prev = localStorage.getItem(STORAGE_KEY);
      if (prev) {
        const t = parseInt(prev, 10);
        if (!isNaN(t) && daysBetween(t) < cfg.hideForDays) return;
      }
    }
  } catch (e) {}
  ...
  function open() {
    ...
    if (cfg.type === 'modal') {
      overlay.hidden = false; panel.hidden = false; overlay.style.opacity = '0'; panel.style.opacity = '0';
      requestAnimationFrame(() => { overlay.style.opacity = '1'; panel.style.opacity = '1'; });
      panel.setAttribute('tabindex','-1'); panel.focus(); trapFocus(panel);
    } else {
      panel.hidden = false; panel.style.transform = 'translateY(18px)'; panel.style.opacity = '0';
      requestAnimationFrame(() => { panel.style.transform = 'translateY(0)'; panel.style.opacity = '1'; });
    }
    if (cfg.autoClose && Number(cfg.autoClose) > 0) setTimeout(close, cfg.autoClose * 1000);
  }
  function close() {
    try { localStorage.setItem(STORAGE_KEY, String(Date.now())); } catch (e) {}
    if (cfg.type === 'modal') {
      overlay.style.opacity = '0'; panel.style.opacity = '0';
      setTimeout(() => { overlay.hidden = true; panel.hidden = true; }, 220);
    } else {
      panel.style.opacity = '0'; panel.style.transform = 'translateY(18px)';
