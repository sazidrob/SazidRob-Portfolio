// js/three-scene.js (robust: will load three.js automatically if missing)
(() => {
  ...
  function ensureThree(ready) {
    if (window.THREE) return ready();
    loadScript('https://unpkg.com/three@0.158.0/build/three.min.js', ready, () => {
      console.error('Could not load Three.js from CDN.');
    });
  }
  ...
  ensureThree(() => {
    (function initScene() {
      ...
      if (window.__siteLoader && typeof window.__siteLoader.hide === 'function') {
        setTimeout(() => window.__siteLoader.hide(), 300);
      }
    })();
  });
})();
